<img alt="adr.xyz" src="https://i.imgur.com/NSBZtuM.png"/>

<center><b>Complete and configurable currency and games multipurpose Discord bot</b></center>

# Using the Official adr.xyz Bot

**Just one [click](https://add.adr.xyz) and you can add adr.xyz to your own server and enjoy its full feature set!**

You can see more information about the bot itself by reading the summary on [here](https://github.com/Mantaro/MantaroBot/blob/master/FEATURES.md). This is the file we use to publish our bots on bot lists, too. You're welcome to use it as a guide for your own. Our [webpage](https://adr.xyz) is hosted in [Github Pages](https://github.com/Mantaro/mantaro.github.io) and outlines most features of the bot, alongside some useful links.

# Building your own adr.xyz

## ⚠ **Read before attempting**
The owners of adr.xyz do not recommend compiling adr.xyz as it is not documented, and most builds you make from pulling the main branch will be extremely unstable and (definitely) untested, probably including unfinished features. There's no *stable* branch, all the features are immediately added to upstream. Yes, really.\
...
You will need the following to utilize all of adr.xyz's features when building your own instance (items marked with a star are optional):
...
### Editing Code:
adr.xyz isn't a modular bot (sadly), but removing features is fairly easy. You can just remove the respective command or the Module file on the commands directory and everything *should* still work. The exception are some Modules that are required by other Modules.
...
### Steps for building:
<sub>Please do note that you will not receive any help whatsoever while trying to make your own adr.xyz build (see above).</sub>
...
This are java arguments that adr.xyz parses outside of config.json at startup and that can be dynamically adjusted using a script, a systemd service or similar:
*   `-Dmantaro.node-number=x` - Specify the number of this node (important if you use more than once instance)
*   `-Dmantaro.shard-count=x` - Specify the amount of shards adr.xyz will be starting up. (for all nodes, see below for subsets)
...
*   `-Dmantaro.debug` - Start adr.xyz with two shards and some extra logging, regardless of the settings above.
...
If you have a self-hosted adr.xyz instance and want to use the `master` branch, run [this](https://github.com/Mantaro/MantaroDatabaseMigrator) program with both MongoDB and RethinkDB on. You can either build it or run the main function on an IDE.
